...
plt.plot(logx, logmix, 'r-', lw=2, alpha=0.8, label='chi2 pdf')
plt.axvline(x=np.log(chi2.ppf(0.95,1)+1))
#plt.plot(x, mix)
plt.hist(logw,bins=100,alpha=0.7,density=True)
plt.hist(logw_null,bins=100,alpha=0.7,density=True)
...
plt.savefig("chi2_null_vs_ha.pdf", format="pdf")
plt.show()

plt.scatter(x=df['pi'],y= (w < chi2.ppf(0.95,1)),alpha=0.5)
plt.show()
